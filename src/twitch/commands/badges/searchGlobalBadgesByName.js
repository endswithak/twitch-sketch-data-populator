...
export default (context) => {
  ...
  //get populator type
  let populatorType = TWITCH_POPULATE_TYPE.BADGES.SEARCH.NAME;
  ...
  .then(data => {
    ...
    let selectedBadge = jsonData.hasOwnProperty(options[OPTIONS.SEARCH_QUERY].trim());

    if (!selectedBadge) {
      return Context().document.showMessage(`Could not find the badge "${options[OPTIONS.SEARCH_QUERY].trim()}"`);
    }

    saveJSON(jsonData[options[OPTIONS.SEARCH_QUERY].trim()]);

    jsonData = loadJSONData(PRESET_DATA_PATH);
