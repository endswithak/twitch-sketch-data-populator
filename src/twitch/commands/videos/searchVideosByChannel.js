...
export default (context) => {
  ...
  //get populator type
  let populatorType = TWITCH_POPULATE_TYPE.VIDEOS.SEARCH.CHANNEL;
  ...
  getChannelId(options[OPTIONS.SEARCH_QUERY].trim()).then(id => {
    twitchApi.get({
      method: `channels/${Number(id)}/videos`,
      params: {
    ...
    .then(data => {
      ...
      if (!jsonData) {
        ...
        return Context().document.showMessage(`"${options[OPTIONS.SEARCH_QUERY].trim()}" does not have any videos`);
      }
