...
export default (context) => {
  ...
  //get populator type
  let populatorType = TWITCH_POPULATE_TYPE.VIDEOS.SEARCH.ID;
  ...
  twitchApi.get({
    method: `videos/${options[OPTIONS.SEARCH_QUERY].trim()}`,
    params: {
      api_version: 5
    }
  })
  .then(data => {
    ...
    if (!jsonData) {
      ...
      return Context().document.showMessage(`No videos found with the id "${options[OPTIONS.SEARCH_QUERY].trim()}"`);
    }
