...
export default (context) => {
  ...
  //get populator type
  let populatorType = TWITCH_POPULATE_TYPE.STREAMS.SEARCH.LANGUAGE;
  ...
  twitchApi.get({
    method: 'streams',
    params: {
      language: encodeURIComponent(options[OPTIONS.SEARCH_QUERY].trim()),
  ...
  .then(data => {
    ...
    if (!jsonData) {
      ...
      return Context().document.showMessage(`No streams found with the language ${options[OPTIONS.SEARCH_QUERY].trim()}`);
    }
