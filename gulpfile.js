...
gulp.task('assemble-twitch-plugin-resources',function(callback) {
    function normalizePluginFileName(name) {
        return name;
    }

    return gulp.src('src/twitch/resources/**/*.*')
        .pipe(gulp.dest(path.join(__dirname,'dist',normalizePluginFileName(currentManifest.bundleName || currentManifest.name)+'.sketchplugin','Contents/Resources')));
});
...
gulp.task('build',function(callback) {
    runSequence('clean','prepare-folders','bundle','prepare-manifest','assemble-plugin-bundle','assemble-plugin-resources','assemble-twitch-plugin-resources','assemble-plugin-presets','install-plugin',callback);
});

gulp.task('build-custom',function(callback) {
    runSequence('clean','assets','prepare-folders','bundle','prepare-manifest','assemble-plugin-bundle','assemble-plugin-resources','assemble-twitch-plugin-resources','assemble-plugin-presets','assemble-plugin-custompresets','install-plugin',callback);
});
...
gulp.task('zip', ['build'], function() {
  return gulp.src('./dist/*.sketchplugin/**/*')
    .pipe(zip('TwitchDataPopulator.zip'))
    .pipe(gulp.dest('dist'))
});

gulp.task('release', ['zip'], function() {
  return gulp.src('./dist/TwitchDataPopulator.zip')
    .pipe(release({
